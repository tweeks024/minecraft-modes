...
/**
 * Obi-Wan Kenobi: humanoid + robe skirt (hangs from the hips over the top
 * half of the legs) + hood (inflated head shell, open at the front) —
 * reuses the Jedi Knight's accessory geometry exactly. Vanilla hat overlay
 * hidden to avoid z-fighting with the hood.
 */
public class ObiWanModel extends HumanoidModel<HumanoidRenderState> {

    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(
        Identifier.fromNamespaceAndPath(StarWarsMod.MOD_ID, "obi_wan"),
        "main");

    public ObiWanModel(ModelPart root) {
        super(root);
