...
/**
 * A single Jedi ruin, generated as a scattered feature.
 *
 ...
 * {@link ImperialOutpostPiece}.
 */
public class JediRuinPiece extends ScatteredFeaturePiece {

    public static final ResourceKey<LootTable> LOOT = ResourceKey.create(
        Registries.LOOT_TABLE,
        Identifier.fromNamespaceAndPath(StarWarsMod.MOD_ID, "chests/jedi_ruin"));

    /**
     * @param minBlockX world X of the chunk's min corner (the ruin's local x=0)
     * @param minBlockZ world Z of the chunk's min corner (the ruin's local z=0)
     */
    public JediRuinPiece(RandomSource random, int minBlockX, int minBlockZ) {
        super(ModStructures.JEDI_RUIN_PIECE.get(), minBlockX, 64, minBlockZ,
            JediRuinLayout.SIZE_X, JediRuinLayout.SIZE_Y, JediRuinLayout.SIZE_Z,
            getRandomHorizontalDirection(random));
    }

    public JediRuinPiece(StructurePieceSerializationContext ctx, CompoundTag tag) {
        super(ModStructures.JEDI_RUIN_PIECE.get(), tag);
    }
    ...
    public void postProcess(WorldGenLevel level, StructureManager structureManager,
        ...
        // blocks; bails out (leaving the ruin ungenerated in this chunk pass)
        ...
        for (var p : JediRuinLayout.placements()) {
            ...
            } else if (p.kind() == JediRuinLayout.Kind.CHEST) {
                this.createChest(level, box, random, p.dx(), p.dy(), p.dz(), LOOT);
            }
        }

        // Guardians: spawn living Jedi knights at generation time. Coordinates are
        // mapped local->world through the oriented box (getWorldPos), so the
        // guardians rotate/mirror with the piece. Skipped if the marker falls
        // outside the current chunk's box (the piece may straddle chunk borders).
        for (var p : JediRuinLayout.placements()) {
            if (p.kind() != JediRuinLayout.Kind.GUARDIAN_JEDI) continue;
            EntityType<? extends Mob> type = ModEntities.JEDI_KNIGHT.get();
