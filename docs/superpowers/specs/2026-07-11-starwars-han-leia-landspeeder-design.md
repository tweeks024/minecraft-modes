...
- Two seats: driver and passenger seated side-by-side in the cockpit (±0.45 lateral offset from hull center), consistently across Java, bbmodel, and Bedrock (deviation from the original draft: side-by-side matches the X-34's actual cockpit; implemented consistently across Java, bbmodel, and Bedrock); `positionRider` offsets derive from the bbmodel geometry. `getMaxPassengers() = 2` (or the version's equivalent hook).
