...
- **`HyperspacePortalBlock`** (first block in the mod; `DeferredRegister.createBlocks`): properties `PLANET` (enum, 4) + `AXIS` (X/Z); thin pane shape, no collision, light 11, unbreakable-by-hand like nether portal, no item form. Implements `Portal`: transition time 20 ticks (players) / 40 (others); `entityInside → setAsInsidePortal`. Neighbor updates dissolve the film when the frame breaks (local frame-or-portal check, cascades). `onRemove` unregisters its gate record.
